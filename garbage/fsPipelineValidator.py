...
class PipelineValidator:
        ...
        try:
            with str(preimport["file_type"]).lower() as ft:
                ...
                if ft == None:
                    ...
                    warn = (
                        []
                    )  # only possible warning so far is one we want to upgrade to fail
                    fail.append(
                        "The glencoe ome.tiff script (raw2ometiff) is missing, either switch to bioformats or add raw2ometiff to your PATH"
                    )
                ...
                if k == "file_type" or k == ~".*comment":
                    ...
                    try:
                        if len(list(preimport[key])) < 1:
                            fail.append(
                                f"Why did you give an empty list to series? Not allowed."
                            )
                    except:
                    ...
                    try:
                        ...
                        fail.append(
                            f"pyramid_resolutions:{preimport[key]} - is not an integer"
                        )
                    ...
                    with str(preimport[key]).upper() as pdt:
                        ...
                        if pdt not in values:
                            fail.append(
                                f"pyramid_downsample_type:{preimport[key]} - is not a supported downsample technique"
                            )
                    ...
                    try:
                        if bool(preimport[key]) == False:
                            warn.append(
                                "Are you sure you don't want to mark this as rgb?"
                            )
                    except:
                    ...
                    warn.append(
                        "extra parameters have no protections, bugs are almost guaranteed"
                    )
                    try:
                        list(preimport[key])
                    except:
                        warn.append(
                            "even if only one set of extra params are provided, put it in an array to be safe"
                        )
                        preimport[key] = [preimport[key]]

                elif k == "pyramid_scale":
                    warn.append(
                        f"pyramid_scale param is Bioformats exclusive and has no effect on Glencoe tools"
                    )

                elif k == "noflat":
                    warn.append(
                        "Noflat param is Bioformats exclusive and has no effect on Glencoe tools"
                    )

                elif k == "bigtiff":
                    warn.append(
                        "bigtiff param is Bioformats exclusive and has no effect on Glencoe tools"
                    )

                elif k == "timepoint":
                    warn.append(
                        "timepoint param is Bioformats exclusive and has no effect on Glencoe tools"
                    )
                ...
                if k == "file_type" or k == ~".*comment":
                    ...
                    with str(preimport[key]).lower() as cf:
                        ...
                        if ft in tiffs:
                            ...
                            warn.append(
                                f"Cannot confirm compression is compatible with desired file type"
                            )
                            values = tiffFormats
                    ...
                    try:
                        if len(list(preimport[key])) != 2:
                            fail.append(
                                f"Only allowed to given an inclusive range to bfconvert's series param"
                            )
                    except:
                        fail.append(f"tile_size_y:{preimport[key]} - is not a list")
                    ...
                    try:
                        ...
                        fail.append(
                            f"pyramid_scale:{preimport[key]} - is not a floating point number"
                        )
                    ...
                    try:
                        if bool(preimport[key]) == False:
                            warn.append(
                                "Are you sure you want to completely disable bigtiff?"
                            )
                    except:
                    ...
                    warn.append(
                        "pyramid_downsample_type is a Glencoe exclusive and will have no effects on Bioformats tools"
                    )
    ...
    def validate(json):
        ...
        if json.preimport == None and json.postimport == None:
            fail.append(
                "No meaningful pipeline defined, just use dropbox or make sure your json is written correct"
            )  # TODO
        #####
        # preimport
        #####
            ...
            with str(json.preimport.converter).lower() as converter:
                if converter == "glencoe":
                    ...
                    if converter != "bioformats":
                        warn.append(
                            "converter not recognized, using default converter (bioformats)"
                        )  # TODO
        ...
        if type(json.postimport) == list:
            ...
            fail.append(
                "postimport formatted improperly, post import should be an array of OMERO.script runs"
            )  # TODO
